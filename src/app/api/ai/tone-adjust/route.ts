...
export async function POST(req: Request) {
  try {
    ...
    if (!tone || !['formal', 'casual'].includes(tone)) {
      return NextResponse.json({ success: false, error: 'tone is required and must be "formal" or "casual"' }, { status: 400 });
    }
    ...
    if (!result.success) {
      return NextResponse.json({ success: false, error: result.error || 'Failed to adjust tone' }, { status: 500 });
    }
